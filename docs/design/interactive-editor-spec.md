...
| D-4 | Rubber-band vs. orbit on empty-space drag (edit mode) | (a) drag = marquee, orbit needs modifier/RMB · (b) marquee needs `Shift`-drag · (c) explicit select-tool sub-mode (old `M` toggle) | **(a)** — Avogadro/CrystalMaker/Blender expectation; (c) is the proven in-house fallback |
