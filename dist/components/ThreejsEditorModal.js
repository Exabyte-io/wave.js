...
exports.ThreejsEditorModal = void 0;
var _made = require("@mat3ra/made");
var _propTypes = _interopRequireDefault(require("prop-types"));
var _react = _interopRequireDefault(require("react"));
var THREE = _interopRequireWildcard(require("three"));
var _RemoveMultipleSelectionGroupCommand = require("three/editor/js/commands/RemoveMultipleSelectionGroupCommand");
var _SetSceneCommand = require("three/editor/js/commands/SetSceneCommand");
var _SubmitMultipleSelectionCommand = require("three/editor/js/commands/SubmitMultipleSelectionCommand");
var _Editor = require("three/editor/js/Editor");
var _Menubar = require("three/editor/js/Menubar");
var _Player = require("three/editor/js/Player");
var _Script = require("three/editor/js/Script");
var _Sidebar = require("three/editor/js/Sidebar");
var _Toolbar = require("three/editor/js/Toolbar");
var _Viewport = require("three/editor/js/Viewport");
...
class ThreejsEditorModal extends _ModalDialog.ModalDialog {
  constructor(props) {
    super(props);
    _defineProperty(this, "initializeCamera", () => {
    ...
    this.showSubmissionMultipleSelectionModal = this.showSubmissionMultipleSelectionModal.bind(this);
  ...
  initializeControlsInEditor() {
    // initialize controls
    this.editor.controls = new _OrbitControls.OrbitControls(this.editor.camera, this.viewport.dom);
  ...
  initializeRaycaster() {
    ...
    const labels = this.editor.scene.getObjectByName(_enums.LABELS_GROUP_NAME).children;
  ...
  initializeEditor() {
    ...
    this.editor = new _Editor.Editor(this.initializeCamera());
    this.editor.scene.background = new THREE.Color(_settings.default.backgroundColor);

    // pass onHide handler to editor
    this.editor.onHide = this.onHide;

    // initialize viewport and add it to the DOM
    this.viewport = new _Viewport.Viewport(this.editor);
    this.domElement.appendChild(this.viewport.dom);

    // initialize UI elements and add them to the DOM
    const script = new _Script.Script(this.editor);
    this.domElement.appendChild(script.dom);
    const player = new _Player.Player(this.editor);
    this.domElement.appendChild(player.dom);
    const menubar = new _Menubar.Menubar(this.editor);
    this.domElement.appendChild(menubar.dom);
    const sidebar = new _Sidebar.Sidebar(this.editor);
    this.domElement.appendChild(sidebar.dom);
    const toolbar = new _Toolbar.Toolbar(this.editor);
  ...
  addEventListeners() {
    ...
    document.addEventListener("mousedown", event => {
      ...
      if (event.button === THREE.MOUSE.LEFT) {
        ...
        const hasMeshIntersect = intersects.some(intersect => intersect.object.type === "Mesh");
  ...
  loadScene() {
    ...
    const scene = loader.parse((0, _utils.materialsToThreeDSceneData)(this.props.materials));
    this.editor.execute(new _SetSceneCommand.SetSceneCommand(this.editor, scene));
  ...
  submitMultipleSelectionGroup() {
    this.editor.execute(new _SubmitMultipleSelectionCommand.SubmitMultipleSelectionCommand(this.editor, this.editor.multipleSelection));
  }
  ...
  removeMultipleSelectionGroup() {
    this.editor.execute(new _RemoveMultipleSelectionGroupCommand.RemoveMultipleSelectionGroupCommand(this.editor));
  }
  ...
  showSubmissionMultipleSelectionModal() {
    this.showAlert({
      title: "Warning!",
      content: "Multiple selection group is not submitted do you want submit and exit or undo latest changes?",
      buttons: [{
        text: "Close",
        onClick: this.alertRef.close
      }, {
        text: "Undo and Exit",
  ...
  extractMaterialAndHide() {
    try {
      const material = (0, _utils.ThreeDSceneDataToMaterial)(this.editor.scene);
  ...
  onExtractMaterialError() {
    this.showAlert({
      ...
      buttons: [{
        ...
        text: "Exit",
        onClick: this.forceExitFromEditor
      }]
    });
  }
  ...
  renderBody() {
    return /*#__PURE__*/_react.default.createElement(_react.default.Fragment, null, /*#__PURE__*/_react.default.createElement(_AlertDialog.AlertDialog, {
      ref: ref => this.alertRef = ref
    }), /*#__PURE__*/_react.default.createElement("div", {
      ref: el => this.initialize(el)
    }));
  }
}
exports.ThreejsEditorModal = ThreejsEditorModal;
ThreejsEditorModal.propTypes = {
  materials: _propTypes.default.arrayOf(_made.Made.Material).isRequired
};