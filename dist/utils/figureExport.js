...
export const FIGURE_BACKGROUNDS = [
    {
        id: "viewer",
        label: "Viewer (dark)",
        clearColor: "#202020",
...
export function getFigureBackground(id) {
...
export function getFigureSizePreset(id) {
 ...
 */
export function getFigureResolution({ presetId, customWidth, customHeight, viewportWidth, viewportHeight, maxDimension = DEFAULT_MAX_FIGURE_DIMENSION, }) {
...
export function getFigurePhysicalSize({ width, height }, dpi = FIGURE_DPI) {
    return {
...
export function describeFigureResolution(resolution) {
...
function niceLengthNear(target) {
    ...
    const candidates = [exponent - 1, exponent, exponent + 1].flatMap((e) => NICE_MANTISSAS.map((mantissa) => mantissa * 10 ** e));
    return candidates.reduce((best, candidate) => Math.abs(Math.log(candidate / target)) < Math.abs(Math.log(best / target))
        ? candidate
        : best);
}
function formatAngstrom(length) {
    // Strip the trailing zeros a fixed precision would leave: "0.5 Å", "2 Å", "20 Å".
    const text = length >= 1 ? String(Number(length.toFixed(2))) : String(Number(length.toFixed(3)));
    return `${text} Å`;
}
/**
 ...
 */
export function getScaleBarPlan({ worldUnitsPerPixel, imageWidth, targetFraction = 0.18, }) {
    if (!Number.isFinite(worldUnitsPerPixel) || worldUnitsPerPixel <= 0)
        return null;
    if (!Number.isFinite(imageWidth) || imageWidth < MIN_FIGURE_DIMENSION)
        return null;
    const targetWorld = worldUnitsPerPixel * imageWidth * targetFraction;
    if (!Number.isFinite(targetWorld) || targetWorld <= 0)
        return null;
    const lengthAngstrom = niceLengthNear(targetWorld);
    const lengthPx = lengthAngstrom / worldUnitsPerPixel;
    if (!Number.isFinite(lengthPx) || lengthPx < 8 || lengthPx > imageWidth * 0.8)
        return null;
 ...
 */
export function getScaleBarLayout({ plan, width, height, }) {
 ...
 */
export function drawScaleBar(context, plan, { width, height, color }) {
 ...
 */
export function getFigureFileName({ name, formula, width, height, backgroundId, }) {
