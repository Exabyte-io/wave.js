...
export function getMeasurementLabel(measurementType) {
...
export function getMeasurementHint(measurement) {
    if (!measurement)
        return "";
    ...
    const nouns = { 2: "two atoms", 3: "three atoms" };
 ...
 */
export function getMeasurementProgress(measurement) {
    const needed = Math.max(1, (measurement === null || measurement === void 0 ? void 0 : measurement.atomsPerMeasurement) || 1);
    const count = (measurement === null || measurement === void 0 ? void 0 : measurement.selectedAtomsCount) || 0;
    const picked = needed > 1 ? count % needed : 0;
    return { picked, needed, isPartial: picked > 0 };
}
const round = (value) => value.toFixed(settings.roundPrecision);
/**
...
export function formatMeasurementValue(measurement) {
    const values = measurement === null || measurement === void 0 ? void 0 : measurement.values;
    if (!Array.isArray(values) || !values.length)
        return null;
    const latest = values[values.length - 1];
    if ((measurement === null || measurement === void 0 ? void 0 : measurement.measurementType) === MEASUREMENT_MODES.COORDINATE) {
        if (!Array.isArray(latest))
            return null;
        return `(${latest.map((component) => round(component)).join(", ")})`;
    }
    if (typeof latest !== "number" || Number.isNaN(latest))
        return null;
    if ((measurement === null || measurement === void 0 ? void 0 : measurement.measurementType) === MEASUREMENT_MODES.ANGLE) {
        return `${round(latest)}°`;
    }
