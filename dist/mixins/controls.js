...
const OrbitControlsMixin = superclass => class extends superclass {
  ...
  initAxes() {
    ...
    const points = [new THREE.Vector3(-length / 2, 0, 0), new THREE.Vector3(length / 2, 0, 0), new THREE.Vector3(0, -length / 2, 0), new THREE.Vector3(0, length / 2, 0), new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, length / 2)];
  ...
  initSecondAxes() {
    ...
    const [x, y, z] = [new THREE.ArrowHelper(new TV3(1, 0, 0), origin, length, "#FF0000", length / 3, length / 3), new THREE.ArrowHelper(new TV3(0, 1, 0), origin, length, "#00FF00", length / 3, length / 3), new THREE.ArrowHelper(new TV3(0, 0, 1), origin, length, "#0000FF", length / 3, length / 3)];
  ...
  addTargetCrossToCamera() {
    const TargetCrossHelper = new THREE.Mesh(new THREE.CircleGeometry(0.2, 32), new THREE.MeshBasicMaterial({
      color: 0xffffff
    }));
 ...
 */
const ControlsMixin = superclass => (0, _utils.UtilsMixin)(OrbitControlsMixin(class extends superclass {
  constructor(config) {
    super(config);
