...
const OrbitControlsMixin = (superclass) => class extends superclass {
    ...
    addTargetCrossToCamera() {
        const TargetCrossHelper = new THREE.Mesh(new THREE.CircleGeometry(0.2, 32), new THREE.MeshBasicMaterial({ color: 0xffffff }));
...
export const ControlsMixin = (superclass) => UtilsMixin(OrbitControlsMixin(class extends superclass {
    ...
    setCameraAlongCellVector(axis) {
        if (!this.orbitControls || !this._cell)
            return;
        ...
        if (!raw || raw.some((component) => !Number.isFinite(component)))
            return;
