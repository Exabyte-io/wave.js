...
import { ANGLE, ATOM_CONNECTION_LINE_NAME, ATOM_CONNECTIONS_GROUP_NAME, ATOM_GROUP_NAME, COLORS, MEASUREMENT_LABELS_GROUP_NAME, MIN_ANGLE_POINTS_DISTANCE, } from "../enums";
...
export const MeasurementMixin = (superclass) => class extends superclass {
    ...
    onPointerMove(event) {
        ...
        for (let i = 0; i < intersects.length; i++) {
            ...
            if (intersectItem.uuid !== ((_a = this.intersected) === null || _a === void 0 ? void 0 : _a.uuid)) {
                this.handleUnsetHex();
                this.intersected = null;
            }
            if (!((_b = intersectItem.userData) === null || _b === void 0 ? void 0 : _b.selected)) {
    ...
    handleConnectionClick(intersectItem) {
        var _a;
        if ((_a = this.currentSelectedLine) === null || _a === void 0 ? void 0 : _a.userData.selected) {
    ...
    checkAtomPairExistence(atomArray, [atomA, atomB]) {
        for (let i = 0; i < atomArray.length; i += 2) {
            ...
            if ((currentAtom.uuid === atomA.uuid && atomB.uuid === nextAtom.uuid) ||
    ...
    getPointsForAngleDraw(atomPositions, angle) {
        ...
        const firstPoint = this.getPointInBetweenByLength(positionB, positionA, MIN_ANGLE_POINTS_DISTANCE);
        const lastPoint = this.getPointInBetweenByLength(positionB, positionC, MIN_ANGLE_POINTS_DISTANCE);
    ...
    deSelectAtom(atom) {
        var _a;
        if (!((_a = atom.userData.connections) === null || _a === void 0 ? void 0 : _a.length)) {
            atom.userData.selected = false;
            this.selectedAtoms.pop();
            atom.material.emissive.setHex(atom.currentHex);
    ...
    addIfLastNotSame(intersectedAtom) {
        ...
        if ((lastAtom === null || lastAtom === void 0 ? void 0 : lastAtom.uuid) === intersectedAtom.uuid && this.selectedAtoms.length % 2) {
    ...
    drawCoordinateText(position, atom) {
        ...
        const label = this.createLabelSprite(`${position.x.toFixed(3)}, ${position.y.toFixed(3)}, ${position.z.toFixed(3)}`, `coordinates-for-${atom.uuid}`, this.settings.coordinateLabelsConfig);
