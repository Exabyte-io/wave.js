...
exports.MeasurementMixin = void 0;
 ...
 */
const MeasurementMixin = superclass => class extends superclass {
  constructor(config) {
    super(config);
    this.initRaycaster();
    this.selectedAtoms = [];
    ...
    this.atomConnections.name = _enums.ATOM_CONNECTIONS_GROUP_NAME;
    this.angles.name = "Angles";
    this.measurementLabels.name = _enums.MEASUREMENT_LABELS_GROUP_NAME;
  }
  destroyListeners() {
  ...
  getAtomGroups() {
    ...
    this.structureGroup.children.forEach(group => {
      if (group.name === _enums.ATOM_GROUP_NAME) {
  ...
  setHexForLine(intersectItem) {
    if (this.intersected !== intersectItem) {
      ...
      this.intersected.material.color.setHex(_enums.COLORS.GREEN);
  ...
  setHexForAtom(intersectItem) {
    if (this.intersected !== intersectItem) {
      ...
      this.intersected.material.emissive.setHex(_enums.COLORS.RED);
      this.renderer.render(this.scene, this.camera);
    }
  }
  ...
  handleUnsetHex() {
    if (this.intersected && !this.intersected.userData.selected) {
      if (this.intersected.name === _enums.ATOM_CONNECTION_LINE_NAME || this.intersected.name === _enums.ANGLE) {
  ...
  onPointerMove(event) {
    ...
    for (let i = 0; i < intersects.length; i++) {
      ...
      if (!intersectItem.userData?.selected) {
        if (intersectItem.name === _enums.ATOM_CONNECTION_LINE_NAME || intersectItem.name === _enums.ANGLE) {
  ...
  deleteConnection() {
    ...
    if (this.currentSelectedLine.name === _enums.ANGLE) {
      return this.deleteConnectionsUsingAngle();
    }
    this.selectedAtoms.forEach((atom, index) => {
      const atomConnections = atom.userData.connections;
      if (!atomConnections) return;
      const isAtomUseThisConnection = atomConnections.some(connection => connection === this.currentSelectedLine.uuid);
      if (isAtomUseThisConnection) {
        atom.userData.connections = atomConnections.filter(connection => connection !== this.currentSelectedLine.uuid);
        this.selectedAtoms[index] = null;
      }
      if (!atom.userData.connections.length) {
        atom.userData.selected = false;
  ...
  onClick(updateState, event) {
    ...
    for (let i = 0; i < intersects.length; i++) {
      ...
      if (intersectItem.type === "Mesh") {
        ...
        if (this.measurementSettings.isDistanceShown) this.addIfLastNotSame(intersectItem);
        if (this.measurementSettings.isAnglesShown) this.addIfTwoLastNotSame(intersectItem);
        ...
        if (this.shouldCalculateDistance()) {
          ...
          const isPairAlreadyExist = this.checkAtomPairExistence(this.selectedAtoms.slice(0, -2), lastselectedAtoms);
  ...
  checkAtomPairExistence(atomArray, [atomA, atomB]) {
    for (let i = 0; i < atomArray.length; i += 2) {
      ...
      if (currentAtom.uuid === atomA.uuid && atomB.uuid === nextAtom.uuid || currentAtom.uuid === atomB.uuid && nextAtom.uuid === atomA.uuid) {
  ...
  shouldCalculateDistance() {
    return this.selectedAtoms.length && !(this.selectedAtoms.length % 2) && this.measurementSettings.isDistanceShown;
  }
  shouldCalculateAngles() {
    return this.selectedAtoms.length && !(this.selectedAtoms.length % 3) && this.measurementSettings.isAnglesShown;
  }
  ...
  getDistanceToControlPointByAngle(angle) {
    return angle / 100 + _enums.MIN_ANGLE_POINTS_DISTANCE;
  }
  ...
  getPointsForAngleDraw(atomPositions, angle) {
    ...
    const firstPoint = this.getPointInBetweenByLength(positionB, positionA, _enums.MIN_ANGLE_POINTS_DISTANCE);
    const lastPoint = this.getPointInBetweenByLength(positionB, positionC, _enums.MIN_ANGLE_POINTS_DISTANCE);
    const firstPointControl = this.getPointInBetweenByLength(positionB, positionA, distanceToControlPoints);
    const secondPointControl = this.getPointInBetweenByLength(positionB, positionC, distanceToControlPoints);
    const centerControlPoint = this.getPointInBetweenByPercentage(firstPointControl, secondPointControl, 0.5);
  ...
  drawLineBetweenAtoms(lastselectedAtoms) {
    ...
    line.name = _enums.ATOM_CONNECTION_LINE_NAME;
...
exports.MeasurementMixin = MeasurementMixin;