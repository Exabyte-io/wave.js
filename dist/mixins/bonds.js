...
exports.BondsMixin = void 0;
var _periodicTable = require("@exabyte-io/periodic-table.js");
var _made = require("@mat3ra/made");
var _staticKdtree = _interopRequireDefault(require("static-kdtree"));
 ...
 */
const BondsMixin = superclass => class extends superclass {
  constructor(config) {
    super(config);
  ...
  areElementsBonded(element1, coordinate1, element2, coordinate2, bondsData) {
    const distance = _made.Made.math.vDist(coordinate1, coordinate2);
    const connectivityFactor = this.settings.chemicalConnectivityFactor;
    return Boolean((0, _periodicTable.filterBondsDataByElementsAndOrder)(bondsData, element1, element2).find(b => {
    ...
    uniqueElements.forEach((element1, index1) => {
      uniqueElements.forEach((element2, index2) => {
        if (element1 && element2 && index2 >= index1) {
          Array.prototype.push.apply(bonds, (0, _periodicTable.getElementsBondsData)(element1, element2));
        }
      });
    });
    return bonds;
  ...
  getMaxBondLength(bondsData) {
    const connectivityFactor = this.settings.chemicalConnectivityFactor;
    return connectivityFactor * _made.Made.math.max(bondsData.map(b => b.length.value || 0));
  }
  ...
  createBondsGroup() {
    ...
    const tree = (0, _staticKdtree.default)(
