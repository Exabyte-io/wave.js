...
export const AtomLabelsMixin = (superclass) => class extends BaseLabelsMixin(superclass) {
    ...
    createAtomLabels() {
        ...
        if (this.settings.atomLabelsConfig.areSpritesUsed) {
            this.createLabelsAsSprites(verticesHashMap, getNameForLabel, this.getLabelOffsetVector.bind(this), (text, position) => ({ atomPosition: position, atomName: text }), this.atomLabelsGroup, this.settings.atomLabelsConfig);
    ...
    adjustAtomLabelsToCameraPosition() {
        if (!this.areAtomLabelsShown || !this.settings.atomLabelsConfig.areSpritesUsed)
            return;
