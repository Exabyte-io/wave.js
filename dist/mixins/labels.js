...
function _assertClassBrand(e, t, n) { if ("function" == typeof e ? e === t : e.has(t)) return arguments.length < 3 ? t : n; throw new TypeError("Private element is not present on this object"); } // eslint-disable-next-line import/no-cycle
/*
 * Mixin containing the logic for dealing with atom labels.
 * Dynamically draws labels over atoms.
 */
const LabelsMixin = superclass => {
  var _texturesCache;
  return _texturesCache = /*#__PURE__*/new WeakMap(), class extends superclass {
    constructor(config) {
      super(config);
      _classPrivateFieldInitSpec(this, _texturesCache, {});
      this.labelsGroup = new THREE.Group();
      this.labelsGroup.name = _enums.LABELS_GROUP_NAME;
    ...
    createLabelTextTexture(text) {
      ...
      (0, _utils.setParameters)(canvas, {
        width: textSizePowOf2,
        height: textSizePowOf2
      });
      const scaledFont = `${fontWeight} ${scaledFontSize}px ${fontFace}`;
      (0, _utils.setParameters)(context, {
        font: scaledFont,
        ...textParams
      });
    ...
    getLabelTextTexture(text) {
      if (_classPrivateFieldGet(_texturesCache, this)[text]) return _classPrivateFieldGet(_texturesCache, this)[text];
      const texture = this.createLabelTextTexture(text);
      _classPrivateFieldGet(_texturesCache, this)[text] = texture;
      return texture;
    }
    ...
    createVerticesHashMap() {
      ...
      this.structureGroup.children.forEach(group => {
        if (group.name !== _enums.ATOM_GROUP_NAME) return;
