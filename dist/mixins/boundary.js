...
const BoundaryMixin = superclass => class extends superclass {
  /**
  ...
  get areNonPeriodicBoundariesPresent() {
    return _enums.BOUNDARY_CONDITIONS.filter(e => e.isNonPeriodic).map(e => e.type).includes(this.boundaryConditions.type);
  }
  ...
  getBoundaryMeshObject(color, coordinates1, coordinates2, coordinates3, coordinates4, zOffset = 0) {
    ...
    const vertices = new Float32Array([coordinates1, coordinates2, coordinates3, coordinates3, coordinates4, coordinates1].map(([x, y, z]) => [x, y, z + zOffset]).flat());
  ...
  get basisWithElementsInsideNonPeriodicBoundaries() {
    const newBasis = new _made.Made.Basis({
      ...this._basis.toJSON(),
      elements: [],
      coordinates: []
    });
    ...
    basisCloneInCrystalCoordinates.elements.forEach((element, index) => {
      ...
      newBasis.addAtom({
        element,
        coordinate: [coord[0], coord[1], _made.Made.math.abs(coord[2]) <= 0.5 ? coord[2] : coord[2] - 1]
      });
    });
...
exports.BoundaryMixin = BoundaryMixin;