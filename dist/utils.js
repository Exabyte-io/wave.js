...
function extractLatticeFromScene(scene) {
    ...
    if (cellObjects.length === 1) {
        ...
        const vertices = convertPositionToVertices(cellObject, cellObject.geometry.attributes.position);
        if (vertices.length !== 24) {
            throw new Error(`extractLatticeFromScene: expected the periodic unit cell object to have 24 vertices, got ${vertices.length}`);
        }
    ...
    if (cellObjects.length === 2) {
        ...
        const vertices = convertPositionToVertices(upCellObject, upCellObject.geometry.attributes.position);
        if (vertices.length !== 16) {
            throw new Error(`extractLatticeFromScene: expected the non-periodic half unit cell object to have 16 vertices, got ${vertices.length}`);
        }
        const a = vertices[1].sub(vertices[0]).toArray();
        const b = vertices[3].sub(vertices[0]).toArray();
    ...
    throw new Error(`extractLatticeFromScene: expected 1 (periodic) or 2 (non-periodic) unit cell LineSegments objects in the structure group, found ${cellObjects.length}`);
}
/**
